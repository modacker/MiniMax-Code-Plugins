...
//   7. authorize gate (test mode auto-approves; decline path covered via
//      lib-authorize-style test — but here we just confirm the gate is
...
//     NOT mocked — test mode auto-approves via process.execArgv detection.
...
describe("handleExport — error paths", () => {
  test("404 when session id not found", async () => {
    ...
    const res = fakeRes();
    await exportRoute.handleExport(
