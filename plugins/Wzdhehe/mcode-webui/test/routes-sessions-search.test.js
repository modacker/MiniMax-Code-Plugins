...
//   - The handler awaits authorize("session.search", ctx). Under
//     `node --test` (auto-detected by authorize.js:136-149) the gate
//     auto-approves — so happy-path tests just verify the search
//     returns the right rows in the right order.
//   - For the decline path we use the exported AUTHORIZE_ACTIONS
//     whitelist directly: a malformed action returns
//     approved:false immediately (no SSE involved).
...
describe("handleSearchSessions — B03 authorize gate", () => {
  test("auto-approves under `node --test` (production modal not involved)", async () => {
    // We rely on the test-mode auto-approve baked into authorize.js
    // (see authorize.js:136-149). If this test runs under --test, it
    // gets approved:true; under non-test it would hang on the SSE
    // modal — but the project standard test invocation is `node
    ...
    assert.equal(res._status, 200, "should NOT 403 under test mode");
    const body = readBody(res);
    assert.equal(body.ok, true);
  });
