...
import {FULLSCREEN_OFF,FULLSCREEN_OVERLAY,requestMode,rejectApi,apiChange,shouldExitOnKey} from './fullscreen-model.mjs';
...
// Canvas fullscreen: request the native Fullscreen API first and fall back to
// a fixed overlay when the page cannot (or is not allowed to) go fullscreen.
// Either way the canvas keeps its pan/zoom interaction and refits once after
// the geometry change; polling re-renders content without leaving fullscreen.
let canvasFullscreen=FULLSCREEN_OFF;
function canvasFullscreenElement(){return document.fullscreenElement??document.webkitFullscreenElement??null;}
function syncCanvasFullscreenButton(){const active=canvasFullscreen!==FULLSCREEN_OFF,button=$('#graph-fullscreen');button.textContent=t(active?'canvasExitFullscreen':'canvasFullscreen');button.setAttribute('aria-pressed',String(active));}
function refitCanvasFullscreen(){if(current){zoomAuto=true;renderGraph();}}
function setCanvasFullscreen(mode){
 if(mode===canvasFullscreen)return;
 canvasFullscreen=mode;
 $('.canvas-panel').classList.toggle('canvas-overlay',mode===FULLSCREEN_OVERLAY);
 syncCanvasFullscreenButton();
 refitCanvasFullscreen();
}
$('#graph-fullscreen').onclick=async()=>{
 ...
 if(canvasFullscreen===FULLSCREEN_OFF){
  ...
  if(typeof request==='function'){try{await request.call(panel);}catch{setCanvasFullscreen(rejectApi(canvasFullscreen));}}
 }else if(canvasFullscreenElement()){try{await (document.exitFullscreen??document.webkitExitFullscreen).call(document);}catch{/* fullscreenchange still syncs the state */}}
 else setCanvasFullscreen(FULLSCREEN_OFF);
};
document.addEventListener('fullscreenchange',()=>{
 const onPanel=canvasFullscreenElement()===$('.canvas-panel');
 setCanvasFullscreen(apiChange(canvasFullscreen,onPanel));
 if(onPanel||!canvasFullscreenElement())refitCanvasFullscreen();
});
document.addEventListener('webkitfullscreenerror',()=>{setCanvasFullscreen(rejectApi(canvasFullscreen));});
document.addEventListener('keydown',e=>{if(shouldExitOnKey(canvasFullscreen,{key:e.key,dialogOpen:!!document.querySelector('dialog[open]')}))setCanvasFullscreen(FULLSCREEN_OFF);});
